...
#define PROGRAMMING_PIN 2 // ground this pin to disable power saving modes

void setup() {
  pinMode(PROGRAMMING_PIN, INPUT_PULLUP);
  if (digitalRead(PROGRAMMING_PIN)){
    USBDevice.detach(); // disable USB if PROGRAMMING_PIN is not pulled down.
  }else{
    Serial.begin(9600);
    delay(200); 
...
void loop() {
  if (Serial.available()) {
    ...
    Serial.print("case '");
    Serial.write(serialbyte);   // prints the character exactly
    Serial.println("':");
